...
public class Main {
    ...
    public static void Login(){
        ...
        try {
            ...
            for (int i = 0; i < usuariosArr.size(); i++) {
                ...
                if (nombre.equals(usuariosArr.get(i).getUsername()) && contra.equals(usuariosArr.get(i).getPassword())) {
                    Log.addToEndFile("log.log.txt", " " + new Date().toString() +"\t---"+ nombre+":"+" Inicio de Sesión exitoso"+ "\n");
                    System.out.println("El nombre de usuario y contraseña son válidos, Bienvenido.");
                    System.out.println("---------------------------------------------------------------------");
                    usuarioLog=nombre;
                    startMenu();
            ...
            Log.addToEndFile("log.log.txt", " " + new Date().toString() +"\t---"+ nombre+":"+" Inicio de Sesión fallido"+ "\n");
    ...
    public static void cargaArchivos(){
        String archivo;
        cargarRestaurante();
        archivo= restauranteArr.get(0).getLoad();

        if (archivo.equals("json")) {
            cargarUsuarios();
            cargarProductos();
            cargarCliente();
            cargarFacturas();
            Login();
        }else if(archivo.equals("bin")){
            //ACA DEBDEN IR LA CARGA DE ARCHIVOS BIN
            cargarUsuariosBin();
            cargarProductosBin();
            cargarClienteBin();
            cargarFacturasBin();
    ...
    public static void cargarUsuarios() {
        ...
        try {
            ...
            for (Usuarios usuario : usuaris) {
                ...
                if (flag) {
                    Log.addToEndFile("log.log.txt", " " + new Date().toString() + "\t---" + "USUARIOS:" + " El usuario " + usuarioRepetido + " ya existe,se omitió el registro." + "\n");
                    continue;
                }
    ...
    public static Object cargarUsuariosBin() {
        try {

            ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream("usuarios.ipcrm"));
            Object info = objectInputStream.readObject();
            usuariosArr = new ArrayList<>();
            usuariosArr.addAll((Collection<? extends Usuarios>) info);
            objectInputStream.close();
            return info;
        } catch (IOException | ClassNotFoundException e) {
            Log.addToEndFile("errors.log.txt", " " + new Date().toString() + "\t---" + "USUARIOS: Error en la carga de usuarios.bin" + "\n");

        }
        return null;
    }

    public static Object cargarProductosBin(){
        try {
            ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream("products.ipcrm"));
            Object info = objectInputStream.readObject();
            productosArr = new ArrayList<>();
            productosArr.addAll((Collection<? extends Productos>) info);
            objectInputStream.close();
            return info;
        } catch (IOException | ClassNotFoundException e) {
            Log.addToEndFile("errors.log.txt", " " + new Date().toString() + "\t---" + "PRODUCTOS: Error en la carga de productos.bin" + "\n");

        }
        return null;
    }

    public static Object cargarClienteBin(){
        try {
            ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream("clients.ipcrm"));
            Object info = objectInputStream.readObject();
            clientesArr = new ArrayList<>();
            clientesArr.addAll((Collection<? extends Clientes>) info);
            objectInputStream.close();
            return info;
        } catch (IOException | ClassNotFoundException e) {
            Log.addToEndFile("errors.log.txt", " " + new Date().toString() + "\t---" + "CLIENTES: Error en la carga de clientes.bin" + "\n");

        }
        return null;
    }

    public static Object cargarFacturasBin() {
        try {
            ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream("invoices.ipcrm"));
            Object info = objectInputStream.readObject();
            facturasArr = new ArrayList<>();
            facturasArr.addAll((Collection<? extends Facturas>) info);
            objectInputStream.close();
            return info;
        } catch (IOException | ClassNotFoundException e) {
            Log.addToEndFile("errors.log.txt", " " + new Date().toString() + "\t---" + "FACTURAS: Error en la carga de facturas.bin" + "\n");
    ...
    public static void cargarProductos() {
        ...
        try{
            ...
            for (Productos prodic : produc) {
                ...
                if (flag) {
                    Log.addToEndFile("log.log.txt", " " + new Date().toString() + "\t---" + "PRODUCTOS:" + " El producto con id " + productoRepetido + " ya existe,se omitió el registro." + "\n");
                    continue;
                }
    ...
    public static void cargarCliente() {
        ...
        try{
            ...
            for (Clientes clie : client) {
                ...
                if (flag) {
                    Log.addToEndFile("log.log.txt", " " + new Date().toString() + "\t---" + "CLIENTES:" + " El cliente con id " + clienteRepetido + " ya existe,se omitió el registro." + "\n");
                    continue;
                }
    ...
    public static void cargarFacturas() {
        ...
        try {
            ...
            for (Facturas fact : factu) {
                ...
                if (flag) {
                    Log.addToEndFile("log.log.txt", " " + new Date().toString() + "\t---" + "FACTURAS:" + " La factura con id " + facturaRepetida + " ya existe,se omitió el registro." + "\n");
                    continue;
                }
